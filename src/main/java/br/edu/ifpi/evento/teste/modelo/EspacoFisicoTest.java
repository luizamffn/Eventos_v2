...
public class EspacoFisicoTest {
	...
	@Before
	public void init() {
		ef1 = EspacoFisicoBuilder.builder().id((long) 1).getEspacoFisico();
		ef2 = EspacoFisicoBuilder.builder().id((long) 2).getEspacoFisico();
	}

	@Test(expected = EnderecoEspacoFisicoException.class)
	public void nao_aceita_espacos_fisicos_pais_com_mesmo_endereco()
			throws EspacoFisicoPaiException, EnderecoEspacoFisicoException {
	...
	public void somento_o_espacos_fisicos_pai_pode_ter_endereco()
			throws EspacoFisicoPaiException, EnderecoEspacoFisicoException {
		Endereco endereco = new Endereco((long) 1);
		ef1.setEndereco(endereco);
	}
	...
	public void espacoFisicoNaoPodeConterAtividadesComHorariosConflitantes() throws DataFimMenorQueDataInicioException,
			EspacoFisicoComAtividadesConflitantes, AtividadeException, AtividadeHorarioForaDoPeriodoDoEvento,
			DataMenorQueAtualException, AtividadeJaPossuiUmEvento, EventoSateliteHorarioForaDoPeriodoDoEvento {
		...
		Evento evento = EventoBuilder.builder().id((long) 1).dataInicio(dataInicio).dataFim(dataFim)
				.organizador(organizador).espacoFisico(ef1).getEvento();

		evento.setEspacoFisico(EspacoFisicoBuilder.builder().id((long) 1).getEspacoFisico());

		EspacoFisico espacoFisico = EspacoFisicoBuilder.builder().id((long) 2).getEspacoFisico();
		...
		Atividade atividade1 = AtividadeCompravelBuilder.builder().id(Long.valueOf(1)).horarioInicio(dataInicialAt1)
				.horarioFim(dataFinalAt1).evento(evento).espacoFisico(ef2).getAtidadeCompravel();

		Atividade atividade2 = AtividadeCompravelBuilder.builder().id(Long.valueOf(2)).horarioInicio(dataInicialAt2)
